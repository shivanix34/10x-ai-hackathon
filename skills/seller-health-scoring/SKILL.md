...
tags: [seller-health, engagement-scoring, monitoring, wellness]
---

# Seller Health Scoring & Monitoring

## Activation Description

**Use this skill when you need to:**
- Calculate seller health scores across engagement dimensions
- Monitor seller wellness trends and detect degradation
- Identify sellers needing support or optimization
- Track seller metrics across response time, catalog, quota, engagement
- Benchmark sellers within peer groups and plan tiers

**Key triggering terms:**
`seller health` | `seller wellness` | `engagement score` | `seller monitoring` | `health metrics` | `seller benchmarking` | `performance tracking`

## Problem Statement

Sellers lack visibility into health metrics:
- No unified view of performance across multiple dimensions
- Manual health calculations time-consuming and error-prone
- Degradation trends invisible until critical threshold reached
- No peer benchmarking or context for improvement
- Operational teams can't prioritize support efforts

## Solution

**Seller Health Score** = (Catalog Score × 0.25) + (Engagement × 0.25) + (Response Efficiency × 0.20) + (Quota Utilization × 0.20) + (Notification Engagement × 0.10)

**Health Dimensions:**
- **Catalog Freshness**: Product update frequency and quality
- **Response Responsiveness**: Avg response time vs. benchmark
- **Engagement Momentum**: Activity trend (improving/stable/declining)
- **Quota Utilization**: Lead consumption vs. allocation
- **Notification Engagement**: Notification open/click rates

## Key Inputs

- Seller profile: plan_tier, category, primary_metrics
- Engagement events: login, response, catalog_update, notification interactions
- Historical performance: response_times, engagement_trends
- Peer benchmarks: plan_tier cohort averages
- Subscription data: plan tier, MRR, active_days
...
```json
{
  ...
  "dimensions": {
    "catalog_quality": 85,
    "engagement": 68,
    "response_efficiency": 75,
    "quota_utilization": 62,
    "notification_engagement": 70
  },
  "trend": "DECLINING",
  "peer_percentile": 65,
  "risk_indicators": [
    "engagement_declining_7_days",
    "response_time_spike"
  ],
  "recommendations": [
    "Process optimization review",
    "Catalog refresh suggested"
  ]
}
```
...
|--------|--------|-------|------|
| Health Calculation Time | 45 min/report | Real-time | 100% faster |
| Early Warning Detection | 5-7 days late | Immediate | Proactive |
| Support Team Efficiency | Manual scoring | Automated scoring | 60% time saved |
| Seller Engagement | Untracked | Visible + trended | Full visibility |
| Intervention Success | 55% | 75% | +36% |

## Implementation References

**Source Code:**
- [seller_health.py](../../ai-services/scoring/seller_health.py) - Health score calculation
- [models.go](../../backend/internal/models/models.go) - Data structures

**API Integration:**
- GraphQL: `query getSellerHealth(sellerId)` → returns health score + dimensions
- WebSocket: `subscribe seller:health:changed` → real-time updates

**Database:**
- sellers: health_score, engagement_score, response_efficiency, quota_utilization
- seller_behavior_state: tracking engagement trends

## Validation Rules

✓ Seller ID must be active/verified
✓ Engagement scores normalized 0-100
✓ Peer benchmarks updated weekly
✓ Trend calculations require minimum 7 days data
✓ No missing values >14 days without flag

## Edge Cases

| Scenario | Handling |
|----------|----------|
| New seller (<7 days) | Use plan-tier cohort average |
| Inactive seller (>90 days) | Mark as DORMANT; suppress scoring |
| Data quality issues | Flag confidence <0.70; manual review |

---
**Status**: ✅ PRODUCTION | **Implementation**: [Seller Health Code](../../ai-services/scoring/seller_health.py)
